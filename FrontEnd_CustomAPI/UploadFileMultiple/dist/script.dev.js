...
var fileExt = ".doc,.docx,.pdf,.odt,.txt,.rtf" + ",.ods,.xls,.xlsm,.xlsx" + ",.ppt,.pptx,.ppts" + ",.jpeg,.jpg,.ai,.png,.bmp,.gif,.ps,.psd,.svg,.tif,.tiff,.ico" + ",.rar,.7z,.deb,.zip,.tar.gz";
...
var Tb8_FileDinhKem_CanCu = function () {
  ...
  var _ = function _(instance) {
    return linkList.get(instance);
  };

  var Tb8_FileDinhKem_CanCuClass =
  /*#__PURE__*/
  function () {
    function Tb8_FileDinhKem_CanCuClass(obj) {
      _classCallCheck(this, Tb8_FileDinhKem_CanCuClass);
      ...
      if (obj != null && obj != undefined) {
        ...
        this.GhiChu = obj.GhiChu; // For Generic Path:
    ...
    _createClass(Tb8_FileDinhKem_CanCuClass, [{
      key: "bind",
      value: function bind() {
...
var CustomLinkList = function () {
  ...
  function del_button_handler(event) {
    ...
    var textarea = $(this).closest(".link-row").find(".file-desc"); //console.log(link);
    //var jqXHR_instance =
    ...
    _(customFileList).listObj = _(customFileList).listObj.filter(function (item) {
      return item.link.attr("href") != link.attr("href");
    });
    link.remove();
    edit.remove();
    textarea.remove();
    $(this).remove(); //console.log(listAjax)
  ...
  function createLinkRow(customFileListIntance) {
    ...
    var a = document.createElement("a"); //a.innerHTML = postedFiles.name;
    ...
    $(btnTrash).bind("click", {
      customFileList: customFileListIntance
    }, del_button_handler);
    return {
      link: $(a),
      edit: $(btnEdit),
  ...
  var CustomLinkListClass =
  /*#__PURE__*/
  function () {
    function CustomLinkListClass(linkListDiv) {
      _classCallCheck(this, CustomLinkListClass);
      ...
      var privateMembers = {
        ...
        listAjax: [],
        listObj: [],
        opace_class: "opace-30"
      };
      priv.set(this, privateMembers);
    }

    _createClass(CustomLinkListClass, [{
      key: "bind",
      value: function bind() {
      ...
      value: function render() {
        ...
        if (list.length > 0) {
          ...
          for (var i = 0; i < list.length; i++) {
            ...
            ret.textarea.val(list[i].note);
            ret.textarea.html(list[i].note);
      ...
      key: "registerNewLink",
      value: function registerNewLink() {
        return createLinkRow(this);
      }
    }, {
      key: "registerLoadingDiv",
      value: function registerLoadingDiv(loadingDiv) {
      ...
      key: "objConstructor",
      get: function get() {
        return _(this).objConstructor;
      },
      set: function set(con) {
        _(this).objConstructor = con;
      }
    }, {
      key: "opace_class",
      get: function get() {
        return _(this).opace_class;
      },
      set: function set(style) {
        _(this).opace_class = style;
      }
    }, {
      key: "listObj",
      get: function get() {
        return _(this).listObj;
      },
      ...
      key: "listAjax",
      get: function get() {
        return _(this).listAjax;
      },
      set: function set(list) {
        _(this).listAjax = list;
      }
    }]);
...
$(".file-multiple-group").each(function () {
  ...
  linklistContext[id_field] = new CustomLinkList(linkListDiv); //********************END_IMPORTANCE**********************//
  //**********************************************************
  ...
  loadingDiv.hide();
  linklistContext[id_field].registerLoadingDiv(loadingDiv); //var opace_class = linklistContext[id_field].opace_class;
  //**********************************************************
  //*********************IMPORTANCE*************************//
  //  Must have this
  ...
  inputFiles.change(function () {
    var _this = $(this); //var baseUrl = $("form").data("url");


    var baseUrl = $("form").data("url");
    var regex = /Index/; //var baseUrl;

    if (regex.test(baseUrl) == true) {
      baseUrl = baseUrl.replace("Index", ""); //url = url.replace("Index", ext);
    ...
    for (var i = 0; i < postedFiles.length; i++) {
      var formData = new FormData(); //formData.append("id", obj_id);
      ...
      $.ajax({
        ...
        beforeSend: function beforeSend(jqXHR) {
          $.xhrPool.push(jqXHR);
          linklistContext[id_field].listAjax.push({
            jqXHR_obj: jqXHR,
            objLink: newLink
          });
          loadingDiv.show();
        ...
        error: function error(jqXHR, textStatus, errorThrown) {
          ...
          linklistContext[id_field].listAjax.splice(index, 1); //console.log(listAjax);
        ...
        success: function success(ret, status, jqXHR) {
          var index = linklistContext[id_field].listAjax.map(function (e) {
            return e.jqXHR_obj;
          }).indexOf(jqXHR);
          var objLink = linklistContext[id_field].listAjax[index].objLink;
          ...
          link.html(filename); //var href = ret.url;

          var relUrl = ret.rel;
          link.attr("target", "_blank");
          link.attr("download", "");
          var newObj = linklistContext[id_field].GetNewObj(); //console.log(newObj);
          ...
          newObj.path = relUrl;
          linklistContext[id_field].listObj.push(newObj); //console.log(linklistContext[id_field].listObj);

          if (index > -1) linklistContext[id_field].listAjax.splice(index, 1); //Remove from ajax pool:
...
linklistContext["Tb8_FileDinhKem_CanCus"].listObj = (void 0).model.Tb8_FileDinhKem_CanCus;
...
linklistContext["Tb8_FileDinhKem_CanCus"].bind(); //after this, you can use the ret for any thing, it is an array of obj{}--> server
