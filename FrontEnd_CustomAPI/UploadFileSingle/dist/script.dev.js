...
$(".file-single-group").each(function () {
  ...
  var desc = fileInput.siblings(".custom-file-description"); //var del_button = $(this).find("button");
...
$(".file-single-group").each(function () {
  ...
  var del_button = $(this).find("button"); //  del button handler:
  ...
  fileInput.change(function () {
    ...
    var postedFile = $(this).prop("files")[0]; //console.log(postedFile);

    var label = $(this).siblings(".custom-file-label");
    var desc = $(this).siblings(".custom-file-description"); //obj_id = parseInt(obj_id);
    ...
    label.removeClass("text-danger"); //      DATA PROCESS:
    // Form data object:

    var formData = new FormData(); //formData.append('id', obj_id);

    formData.append('file', postedFile); //$.getJSON(reqUrl, { id: obj_id, file: postedFile }, function (ret) {
    //    console.log(ret);
    ...
    del_button.show(); //console.log(reqUrl);
    ...
    jqXHR_instance = $.ajax({
      ...
      success: function success(ret, status, jqXHR) {
        ...
        del_button.siblings("a").find(".file-loading-div").hide(); //Ajax handler:
